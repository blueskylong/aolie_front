    ...
    $(function () {
        ...
        $(document)
            ...
            .on('dnd_start.vakata.jstree', function (e, data) {
                ...
                if (data.data.origin.settings.dnd.isCanDrag) {
                    if (!data.data.origin.settings.dnd.isCanDrag(data)) {
                        return;
                    }
                }
            ...
            .on('dnd_move.vakata.jstree', function (e, data) {
                ...
                let sourceTree = $.jstree.reference(data.element);
                if (sourceTree && sourceTree.settings && sourceTree.settings.dnd) {
                    if (sourceTree.settings.dnd.onlyDroppable) {
                        if ($(data.event.target).hasClass("droppable")) {
                            lastmv = {'ins': null, 'par': $.jstree.root, 'pos': 'last', "isCanDrop": true};
                            marker.hide();
                ...
                if (ins && ins._data && ins._data.dnd) {
                    ...
                    if ((data.event.target === ins.element[0] || data.event.target === ins.get_container_ul()[0]) && ins.get_container_ul().children().length === 0) {
                        ...
                        if (ok) {
                            lastmv = {'ins': ins, 'par': $.jstree.root, 'pos': 'last', "isCanDrop": true};
                            marker.hide();
                            data.helper.find('.jstree-icon').first().removeClass('jstree-er').addClass('jstree-ok');
                            if (data.event.originalEvent && data.event.originalEvent.dataTransfer) {
                                data.event.originalEvent.dataTransfer.dropEffect = is_copy ? 'copy' : 'move';
                            ...
                            if (o === true) {
                                ...
                                if (sourceTree.settings.dnd.isCanDrop) {
                                    if (!sourceTree.settings.dnd.isCanDrop(
                                        ...
                                        marker.hide();
                                        if (lastmv) {
                                            lastmv.isCanDrop = false;
                                        }
                                        return;
                                    } else {
                                        lastmv.isCanDrop = true;
                                    }
                                }
                                return;
                            }
